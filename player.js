class CodeRadio {
    ...
    get played_at() { return this._songStartedAt; }
    ...
    getNowPlaying() {
        ...
        fetch(`https://coderadio-admin.freecodecamp.org/api/nowplaying?t=${new Date().valueOf()}`)
            .then(req => req.json())
            .then(np => {
                ...
                if (this.url === "") this.url = np.station.mounts.find(mount => !!mount.is_default).url;

                // We only need to update th metadata if the song has been changed
                if (!this.currentSong.id || np.now_playing.song.id !== this.currentSong.id) {
                    this.currentSong = np.now_playing.song;
                    this.played_at = np.now_playing.played_at;
                    this.duration = np.now_playing.duration;
                    this.meta.listeners.textContent = `Listeners: ${np.listeners.current}`;
                    if (!this._progressInterval) {
                        this._progressInterval = setInterval(() => this.updateProgress(), 100);
                    }
                }
    ...
    */
    drawVisualizer() {
        if (this.eq.bands.reduce((a, b) => a + b, 0) !== 0) requestAnimationFrame(() => this.drawVisualizer());     // Because timeupdate events are not triggered at browser speed, we use requestanimationframe for higher framerates
        else setTimeout(() => this.drawVisualizer(), 250);                                                          // If there is no music or audio in the song, then reduce the FPS
    
        let y, x = 0;                                                                                                  // Intial bar x coordinate
    ...
    */
    ...
    fadeDown()  { return this.fade('down'); }
    fadeUp()    { return this.fade('up');   }
    ...
    updateVolume() {
        ...
        // Unmet audio volume settings require it to be changed
            ...
            this._player.volume += (this.audioConfig.targetVolume > this._player.volume) ? volumeNextIncrement : -volumeNextIncrement;
